...
Crie uma aplicacao Quart, inicialize `SocketIO` e registre eventos com
decorators.
...
Tambem e possivel criar a extensao sem app e inicializar depois:
...
Handlers Socket.IO podem acessar contexto Quart criado pelo PulseIo durante o
evento. O codigo cria um `Request` com `sid` e `namespace` dinamicos para que
